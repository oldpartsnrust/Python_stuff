...
def tunnelBuild(tunnel, zone, site, count, interfaceIP, interface, publicIP):
    ...
    if backup == 1:
        ...
        p = "!"
    if routerVPN == 1:
        return print("interface Tunnel "+str(count)+str(tunnel)+"\n\
 "+str(p)+"\n\
...
def addEEM (backup, count, TEP):
    ...
    if backup == 1:
        return print ('track 102 stub-object\n\
...
def defaultInterfaces(DialInt, backup):
    if backup == 2:
        return print("interface "+str(DialInt)+"\n\
shutdown\n\
exit\n\
...
# Removes all non-needed routes and interfaces -
def remove(backup):
    if backup == 1:
        if r1 == 1:
            print("no ip route 10.175.183.1 255.255.255.255 Dialer2 250 name DBU-Testing-Route")
        if r2 == 1:
            print("no ip route 167.16.0.0 255.255.255.0 Dialer1 250 name Dial-Route")
        if r3 == 1:
            print("no ip route 204.194.120.0 255.255.248.0 Dialer1 250 name Dial-Route")
        if r4 == 1:
            print("no ip route 204.194.128.0 255.255.240.0 Dialer1 250 name Dial-Route")
        if r5 == 1:
            print("no ip route 206.201.48.0 255.255.240.0 Dialer1 250 name Dial-Route")
        
    if backup == 1 or backup == 2:
        if e1 == 1:
            print("no event manager applet 4GBackupUP")
        if e2 == 1:
            print("no event manager applet 4GBackupDOWN")
        if d1 == 1:
            print("int dialer 1\n\
shutdown\n\
no int dialer 1")
...
no int dialer 2")
        if wl10 == 1:
            print("no dialer watch-list 10")
        if adi == 1:
            print("no ip access-list standard Dbu-In")
        if ado == 1:
            print("no ip access-list standard Dbu-Out")
        if adti == 1:
            print("ip access-list standard Dbutest-In")
        if adto == 1:
            print("no access-list standard Dbutest-Out")
        if a101 == 1:
            print("no access-list 101")
        if dl1 == 1:
            print("no dialer-list 1")
...
def convert(hostNameNapalm, userNameNapalm, passWordNapalm):
    ...
    device = driver(hostname=(hostNameNapalm), username=(userNameNapalm), password=(passWordNapalm),
                    optional_args=(errors))
    print('Opening...')
    device.open()
...
userNameNapalm = input("Enter your HPNA Username: ")
passWordNapalm = input("Enter your RSA Password: ")
...
Please enter a low bandwidth tunnel IP (200 - 203). ")
    ...
    if (findInList(mpls, str(newIPlist[2]))) == 1:
        ...
        continue
    if int(newIPlist[3]) < 0 or int(newIPlist[3]) > 255:
        print ("Please enter a valid IP, your 4th octet is invalid. ")
        continue
...
##############################################################
#####    Open Device, Pull config and run checks         #####
##############################################################
driver = get_network_driver('ios')
dev = driver(hostname=hostNameNapalm, username=(input("Enter your username: ")),
                       password=(input('Enter your RSA Password: ')))
dev.open()

config = str(dev.get_config())
dev.close()
if "interface Dialer2" in config:
    d2 = 1
if "interface Async0" in config:
    a0 = 1
if "interface Async3" in config:
    a3 = 1
if "dialer watch-list 10" in config:
    wl10 = 1
if "ip access-list standard Dbu-In" in config:
    adi = 1
if "ip access-list standard Dbu-Out" in config:
    ado = 1
if "ip access-list standard Dbutest-In" in config:
    adti = 1
if "ip access-list standard Dbutest-Out" in config:
    adto = 1
if "access-list 101" in config:
    a101 = 1
if "dialer-list 1" in config:
    dl1 = 1
if "ip route 10.175.183.1 255.255.255.255 Dialer2 250 name DBU-Testing-Route" in config:
    r1 = 1
if "ip route 167.16.0.0 255.255.255.0 Dialer1 250 name Dial-Route" in config:
    r2 = 1
if "ip route 204.194.120.0 255.255.248.0 Dialer1 250 name Dial-Route" in config:
    r3 = 1
if "ip route 204.194.128.0 255.255.240.0 Dialer1 250 name Dial-Route" in config:
    r4 = 1
if "ip route 206.201.48.0 255.255.240.0 Dialer1 250 name Dial-Route" in config:
    r5 = 1
if "no event manager applet 4GBackupUP" in config:
    e1 = 1
if "event manager applet 4GBackupDOWN" in config:
    e2 = 1    
if "interface Dialer1" in config:
    d1 = 1
    config_list = (config.split('\\n'))
    item = config_list.index("interface Dialer1")
    desc = config_list[item + 1]
    if "description async" not in (config_list[item + 1]):
         if "description" in (config_list[item + 1]):
            desc_updated = desc.replace('description', 'description async')
         else:
            desc_updated = "description async"
